...
#Region "Private fields"
	...
	Private _virtualDevices() As NVirtualDevice
	Private _virtualDeviceManager As VirtualDeviceManagerForm
...
#Region "Private methods"
	...
	Private Sub OnDeviceManagerChanged()
		If _deviceManager Is Nothing Then
			UpdateDeviceList()
		Else
			SyncLock _deviceManager.Devices.SyncRoot
				deviceTreeView.BeginUpdate()
				Try
					UpdateDeviceList()
				Finally
					deviceTreeView.EndUpdate()
				End Try
				AddHandler _deviceManager.DevicesChanging, AddressOf deviceManager_DevicesChanging
				AddHandler _deviceManager.DevicesChanged, AddressOf deviceManager_DevicesChanged
				AddHandler _deviceManager.DeviceAdded, AddressOf deviceManager_DeviceAdded
				AddHandler _deviceManager.DeviceRemoved, AddressOf deviceManager_DeviceRemoved
				AddHandler _deviceManager.DevicesRefreshed, AddressOf deviceManager_DevicesRefreshed
			End SyncLock
		End If
		refreshToolStripMenuItem.Enabled = _deviceManager IsNot Nothing
		closeToolStripMenuItem.Enabled = refreshToolStripMenuItem.Enabled
		connectToolStripMenuItem.Enabled = _deviceManager IsNot Nothing
		If _deviceManager Is Nothing Then
			Text = AppName
		Else
			Text = String.Format("{0} (Device types: {1}, Auto update: {2})", AppName, _deviceManager.DeviceTypes, _deviceManager.AutoUpdate)
		End If
	End Sub
	...
	Private Sub SetDeviceManager(ByVal value As NDeviceManager)
		...
		End If
		If _deviceManager IsNot Nothing Then
			RemoveHandler _deviceManager.DevicesChanging, AddressOf deviceManager_DevicesChanging
			RemoveHandler _deviceManager.DevicesChanged, AddressOf deviceManager_DevicesChanged
			RemoveHandler _deviceManager.DeviceAdded, AddressOf deviceManager_DeviceAdded
			RemoveHandler _deviceManager.DeviceRemoved, AddressOf deviceManager_DeviceRemoved
			RemoveHandler _deviceManager.DevicesRefreshed, AddressOf deviceManager_DevicesRefreshed
	...
	Private Sub NewDeviceManager()
		...
		form.AutoUpdate = settings.AutoUpdate
		If form.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
			SetDeviceManager(New NDeviceManager(form.DeviceTypes, form.AutoPlug, form.AutoUpdate, CType(Nothing, SynchronizationContext)))
...
#Region "Private form events"
	...
	Private Sub deviceManager_DevicesChanging(ByVal sender As Object, ByVal e As EventArgs)
	...
	Private Sub deviceManager_DevicesChanged(ByVal sender As Object, ByVal e As EventArgs)
		BeginInvoke(New MethodInvoker(AddressOf AnonymousMethod2))
	End Sub
	Private Sub AnonymousMethod2()
		deviceTreeView.EndUpdate()
	End Sub

	Private Sub deviceManager_DeviceAdded(ByVal sender As Object, ByVal e As NDeviceManagerDeviceEventArgs)
		BeginInvoke(New Action(Of NDevice)(AddressOf AnonymousMethod3), e.Device)
	End Sub
	Private Sub AnonymousMethod3(ByVal device As NDevice)
		AddDevice(device)
		If deviceTreeView.SelectedNode Is Nothing Then
			deviceTreeView.SelectedNode = deviceTreeView.Nodes(0)
		End If
	...
	Private Sub deviceManager_DeviceRemoved(ByVal sender As Object, ByVal e As NDeviceManagerDeviceEventArgs)
		BeginInvoke(New Action(Of NDevice)(AddressOf AnonymousMethod4), e.Device)
	End Sub
	Private Sub AnonymousMethod4(ByVal device As NDevice)
		RemoveDevice(device)
	End Sub

	Private Sub deviceManager_DevicesRefreshed(ByVal sender As Object, ByVal e As EventArgs)
		BeginInvoke(New MethodInvoker(AddressOf AnonymousMethod5))
	End Sub
	...
	Private Sub deviceCaptureButton_Click(ByVal sender As Object, ByVal e As EventArgs) Handles deviceCaptureButton.Click
		...
		ElseIf biometricDevice IsNot Nothing Then
			...
			If cbUseTimeout.Checked Then
				biometricDeviceForm.Timeout = CInt(Fix(UInteger.Parse(tbMiliseconds.Text)))
			Else
				biometricDeviceForm.Timeout = -1
			End If
		...
		AddHandler form.FormClosed, AddressOf AnonymousMethod6
		form.Show()
	End Sub
	Private Sub AnonymousMethod6(ByVal fcSender As Object, ByVal fcE As FormClosedEventArgs)
	...
	Private Sub virtualDeviceManagerToolStripMenuItem_Click(ByVal sender As Object, ByVal e As EventArgs) Handles virtualDeviceManagerToolStripMenuItem.Click
		If _virtualDeviceManager Is Nothing Then
			_virtualDeviceManager = New VirtualDeviceManagerForm()
			AddHandler _virtualDeviceManager.FormClosing, AddressOf VirtualDeviceManagerClosing
			_virtualDeviceManager.VirtualDevices = _virtualDevices
			_virtualDeviceManager.Show()
		Else
			_virtualDeviceManager.Focus()
		End If

	End Sub

	Private Sub VirtualDeviceManagerClosing(ByVal sender As Object, ByVal e As FormClosingEventArgs)
		Dim form As VirtualDeviceManagerForm = CType(sender, VirtualDeviceManagerForm)
		RemoveHandler form.FormClosing, AddressOf VirtualDeviceManagerClosing
		_virtualDevices = form.VirtualDevices
		_virtualDeviceManager = Nothing
	End Sub
