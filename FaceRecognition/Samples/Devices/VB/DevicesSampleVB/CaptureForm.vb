...
#Region "Protected methods"
	...
	Protected Overridable Sub OnStatusChanged()
		SyncLock _statusLock
			...
			closeButton.Text = CType(IIf(_isCapturing, "Cancel", "Close"), String)
			closeButton.DialogResult = CType(IIf(_isCapturing, System.Windows.Forms.DialogResult.Cancel, System.Windows.Forms.DialogResult.OK), System.Windows.Forms.DialogResult)
			AcceptButton = CType(IIf(_isCapturing, Nothing, closeButton), Button)
		End SyncLock
	End Sub
			...
			If (Not isFinal) Then
				...
				If (elapsed.Subtract(_lastReportTime)).TotalSeconds >= 0.3 Then
					...
					Do
						...
						_timestamps.Dequeue()
					Loop
					If s > Double.Epsilon Then
						_fps = CInt(Fix(Math.Round(_timestamps.Count / s)))
					Else
						_fps = 0
					End If
