...
typedef std::list<wxNIndexPair> SpanningTree;

DECLARE_EVENT_TYPE(wxEVT_MINUTIA_SELECTION_CHANGED, -1)

class wxNFView: public Neurotec::Gui::wxNView
{
private:
	enum
	{
...
public:
		...
		: wxNView(parent, winid)
	{
		...
		this->Connect(ID_DRAW_FEATURES, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(wxNFView::OnToggleDrawFeatures), NULL, this);
		this->Connect(ID_SHOWN_IMAGE_ORIGINAL, ID_SHOWN_IMAGE_PROCESSED, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(wxNFView::OnChooseShownImage), NULL, this);
	}

	virtual ~wxNFView()
	{
		this->Disconnect(ID_DRAW_FEATURES, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(wxNFView::OnToggleDrawFeatures), NULL, this);
		this->Disconnect(ID_SHOWN_IMAGE_ORIGINAL, ID_SHOWN_IMAGE_PROCESSED, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(wxNFView::OnChooseShownImage), NULL, this);
	}
	...
	void SetRecord(Neurotec::Biometrics::wxNFRecord * pRecord)
	{
		m_pRecord = pRecord;
		if (pRecord)
		{
	...
	Neurotec::Biometrics::wxNFRecord* GetRecord()
	{
		return m_pRecord;
	}
	...
	static SpanningTree CalculateSpanningTree(wxNFRecord *record, const std::list<wxNIndexPair> * matedMinutiae = NULL)
	{
		std::list<wxNIndexPair> results;
		...
		if (matedMinutiae != NULL)
		{
			...
			for (std::list<wxNIndexPair>::const_iterator it = matedMinutiae->begin(); it != matedMinutiae->end(); it++)
			{
		...
		{
			record->GetMinutiae()->GetAll(minutiae);
	...
	static SpanningTree InverseSpanningTree(wxNFRecord *record, NMMatchDetails *details, const SpanningTree & spanningTree)
	{
		std::list<wxNIndexPair> results;

		if (!details
			|| !details->FingersMatchDetails
			|| details->FingersMatchDetails->FingerCount != 1
			|| !details->FingersMatchDetails->Fingers[0].NfmMatchDetails)
		{
			return results;
		}
		...
		NfmMatchDetails *fingerDetails = details->FingersMatchDetails->Fingers[0].NfmMatchDetails;
		for (int i = 0; i < minutiaCount; i++)
		{
			indexRemap[i] = -1;
		}
		for (int i = 0; i < fingerDetails->MatedMinutiaCount; i++)
		{
			wxNIndexPair *indexPair = static_cast<wxNIndexPair *>(&fingerDetails->MatedMinutiae[i]);
			indexRemap[indexPair->Index1] = indexPair->Index2;
		}

		for (std::list<wxNIndexPair>::const_iterator it = spanningTree.begin(); it != spanningTree.end(); it++)
		{
			wxNIndexPair remappedPair;
	...
	static SpanningTree InverseSpanningTree(wxNFRecord *record, wxNMatchingDetails * pDetails, const SpanningTree & spanningTree)
	{
		std::list<wxNIndexPair> results;
		...
		std::list<wxNIndexPair> matedMinutiae = GetMatedMinutiae(pDetails);
		for (std::list<wxNIndexPair>::iterator it = matedMinutiae.begin(); it != matedMinutiae.end(); it++)
		{
	...
	static std::list<wxNIndexPair> GetMatedMinutiae(wxNMatchingDetails * pDetails)
	{
		std::list<wxNIndexPair> results;
		...
		wxNFMatchingDetails * pFingerDetails = pDetails->GetFingers()->Get(0);
		int pairCount = 0;
		::std::auto_ptr<wxNIndexPair> pairs(pFingerDetails->GetMatedMinutiae(&pairCount));
	...
	static std::list<wxNIndexPair> GetMatedMinutiae(NMMatchDetails *details)
	{
		std::list<wxNIndexPair> results;
		...
		for (int i = 0; i < fingerDetails->MatedMinutiaCount; i++)
		{
			results.push_back(fingerDetails->MatedMinutiae[i]);
		}
...
private:
	...
	virtual void OnDraw(wxGraphicsContext *gc)
	{
		...
		if (m_drawFeatures && m_pRecord)
		{
			...
			if (m_selectedMinutia >= 0)
			{
				...
				{
					m_pRecord->GetMinutiaeNeighbors()->GetAll(m_selectedMinutia, minutiaNeighbors);
				}
			}
	...
	Neurotec::Biometrics::wxNFRecord *m_pRecord;
