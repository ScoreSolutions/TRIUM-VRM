...
class N_CLASS(NMonitor) : private Internal::NMonitor
{
...
class N_CLASS(NMonitorLocker)
{
	N_DECLARE_NON_COPYABLE(NMonitorLocker)

private:
	N_CLASS(NMonitor) & monitor;

public:
	N_CLASS(NMonitorLocker)(N_CLASS(NMonitor) & monitor)
		: monitor(monitor)
	{
		monitor.Enter();
	}

	~N_CLASS(NMonitorLocker)()
	{
		monitor.Exit();
	}
};
...
#else
	typedef N_CLASS(NMonitor) NMonitorType;
	typedef N_CLASS(NMonitorLocker) NMonitorLockerType;
