...
protected:
	void closeEvent(QCloseEvent *)
	{
		if(pluginManager)
		{
			...
			{
				Neurotec::Plugins::QNPlugin * plugin = pluginManager->GetPlugins()->Get(i);
				plugin->RemoveChangedCallback(PuginChanged, this);
			}
...
private:
	...
	void AddPlugin(Neurotec::Plugins::QNPlugin * plugin)
	{
		...
		try
		{
			plugin->AddChangedCallback(PuginChanged, this);
	...
	void UpdateTotalInfo()
	{
		if (pluginManager)
		{
			...
			parse = parse.arg(QString::number(QTimeTotalSeconds(totalTime), 'g', 2),
							  ...
							  QString::number(pluginManager->GetPlugins()->GetCount()),
	...
	static void PluginManager_DisabledPluginCanged(Neurotec::QNObject * pObject, void * pParam)
	{
		QPluginManagerDialog * dialog = (QPluginManagerDialog*)pParam;
		QMetaObject::invokeMethod(dialog, "OnDisabledPluginsChanged", Qt::QueuedConnection);
		Q_UNUSED(pObject);
	}

	static void PluginManager_PluginAdded(Neurotec::Plugins::QNPluginManager * pPluginManager, Neurotec::Plugins::QNPlugin * pPlugin, void * pParam)
	{
		QPluginManagerDialog * dialog = (QPluginManagerDialog*)pParam;
		QMetaObject::invokeMethod(dialog, "OnPluginAdded", Qt::QueuedConnection, Q_ARG(Neurotec::Plugins::QNPlugin*, pPlugin));
		Q_UNUSED(pPluginManager);
	}

	static void PuginChanged(Neurotec::QNObject * pObject, void * pParam)
	{
		QPluginManagerDialog * dialog = (QPluginManagerDialog*)pParam;
