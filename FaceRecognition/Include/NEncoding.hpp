...
class N_CLASS(NEncodingEx)
{
public:
	static N_CLASS(NType) * NativeTypeOf()
	{
		return N_CLASS(NObject)::GetObject<N_CLASS(NType)>(N_TYPE_OF(NEncoding), true, true);
	}
	...
	static N_CLASS(NString) GetString(NEncoding encoding, const void * pData, NSizeType dataSize)
	{
		HNString hValue;
	...
	static NSizeType GetBytes(NEncoding encoding, const N_CLASS(NStringWrapper) & value, void * pData, NSizeType dataSize)
	{
		NSizeType dataUsed;
