...
class wxNEView: public Neurotec::Gui::wxNView
{
public:
	...
	virtual void OnDraw(wxGraphicsContext *gc)
	{
		...
		if (m_details.OuterBoundaryAvailable)
		{
			...
			::Neurotec::Geometry::wxNPoint *points = static_cast< ::Neurotec::Geometry::wxNPoint *>(&m_details.OuterBoundaryPoints[0]);
		...
		if (m_details.InnerBoundaryAvailable)
		{
			...
			::Neurotec::Geometry::wxNPoint *points = static_cast< ::Neurotec::Geometry::wxNPoint *>(&m_details.InnerBoundaryPoints[0]);
	...
	virtual void OnDraw(wxDC &dc)
	{
		...
		if (m_details.OuterBoundaryAvailable)
		{
			...
			::Neurotec::Geometry::wxNPoint *points = m_details.OuterBoundaryPoints;
		...
		if (m_details.InnerBoundaryAvailable)
		{
			...
			::Neurotec::Geometry::wxNPoint *points = m_details.InnerBoundaryPoints;
			for (int i = 1; i < NEE_BOUNDARY_POINT_COUNT; ++i)
			{
				dc.DrawLine(wxPoint(points[i - 1].X, points[i - 1].Y), wxPoint(points[i].X, points[i].Y));
			}
